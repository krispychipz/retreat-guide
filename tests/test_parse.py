...
<div class="view-content row">
  <div class="card-event">
      <div class="card-title">Spring Sesshin</div>
      <div class="card-date">March 3</div>
      <div class="card-description">Intensive.</div>
      <a href="https://example.com/sesshin">Details</a>
  </div>
</div>
'''
...
def test_fetch_retreat_events_sesshin(monkeypatch):
    class MockResponse:
        def __init__(self, text):
            self.text = text
    ...
    def mock_get(url):
        return MockResponse(SAMPLE_HTML_SESSHIN)

    monkeypatch.setattr("requests.get", mock_get)
    events = fetch_retreat_events("https://dummy")
    assert len(events) == 1
    assert events[0]["title"] == "Spring Sesshin"
